...
class OpsService:
        ...
        if not wait_for_completion:
            return {
                **_transition("generation_requested", event, "check_generation_status"),
                "generation": {
                    "status": generation_task.status if generation_task is not None else "running",
                    "event_id": event["id"],
                    "experiment_id": experiment_id,
                    "generation_task_id": generation_task.task_id if generation_task is not None else None,
                },
            }
    ...
    async def complete_generation_request(
        ...
        *,
        ...
        try:
            if self.generation_service is not None and payload.get("generation_task_id"):
                task = await self.generation_service.wait_for_task(payload["generation_task_id"])
                if task.status == "failed":
                    raise _ops_error_from_generation_task(task, operation_context)
                if task.status != "completed" or task.result is None:
                    raise OpsError(
                        "generation_not_completed",
                        f"Generation task {task.task_id} is {task.status}.",
                    )
                asset_ref = _asset_ref_from_generation_result(task.result)
            else:
                generation_params = _generation_params_for_approved_draft(
                    pipeline=pipeline,
            ...
            error_payload = _generation_error_payload(exc, operation_context)
            ...
            raise OpsError("generation_failed", error_payload["message"]) from exc
        ...
        event = self.store.append_event(
            ...
            payload={
                "asset_ref": asset_ref,
                "generation_task_id": asset_ref.get("generation_task_id"),
            },
            source=source,
        )
    ...
    def get_generation_status(self, experiment_id: str) -> dict[str, Any]:
        ...
        if latest_generation_event["event_type"] == OpsEventType.GENERATION_FAILED.value:
            error_payload = latest_generation_event["payload"].get("error") or latest_generation_event["payload"]
            return {
                "status": "failed",
                "experiment_id": experiment_id,
                "event": latest_generation_event,
        ...
        generation_task_id = latest_generation_event["payload"].get("generation_task_id")
        if self.generation_service is not None and generation_task_id:
            task = self.generation_service.get_task(generation_task_id)
            if task.status == "failed":
                return {
                    "status": "failed",
                    "experiment_id": experiment_id,
                    "event": latest_generation_event,
                    "generation_task": task.model_dump(mode="json"),
    ...
    async def _list_available_pipelines(self) -> tuple[str, ...] | None:
        ...
        if self.generation_service is not None:
            return tuple(self.generation_service.pipeline_registry.pipeline_ids())
...
def _generation_request_for_ops_draft(
    *,
    text: str,
    pipeline: str,
    title: str | None,
    ...
    if pipeline == "asset_based" and params.get("assets"):
        entry = "assets"
        input_payload = {"assets": params.pop("assets")}
        params.setdefault("intent", text)
        if title:
...
def _asset_ref_from_generation_result(result: Any) -> dict[str, Any]:
    ...
    asset_ref = {
        ...
        "path": primary_video.path,
        "url": primary_video.url,
        "media_type": primary_video.media_type,
...
def _ops_error_from_generation_task(task: Any, operation_context: dict[str, Any]) -> OpsError:
    error = task.error.model_dump(mode="json") if task.error else {}
    message = error.get("message") or f"Generation task {task.task_id} failed."
    return OpsError(
...
def _generation_error_payload(exc: Exception, operation_context: dict[str, Any]) -> dict[str, Any]:
    if isinstance(exc, OpsError) and exc.code == "generation_task_failed":
        try:
            payload = json.loads(exc.message)
            payload.setdefault("operation_context", operation_context)
            if payload.get("exception_type") and not payload.get("error_type"):
                payload["error_type"] = payload["exception_type"]
            if payload.get("error_type") and not payload.get("exception_type"):
                payload["exception_type"] = payload["error_type"]
            return payload
