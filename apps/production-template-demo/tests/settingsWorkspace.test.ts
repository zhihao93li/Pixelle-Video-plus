...
test("settings deep links resolve legacy tabs and sections to the new IA", () => {
  assert.deepEqual(
    resolveSettingsLocation(new URLSearchParams("tab=general&section=tts")),
    {
      view: "ai-voice",
      focus: "tts",
      template: null,
  ...
  assert.equal(
    resolveSettingsLocation(
      new URLSearchParams("tab=templates&template=recipe-1")
    ).canonicalPath,
    "/settings?view=recipes&template=recipe-1"
  )
  assert.equal(
    resolveSettingsLocation(new URLSearchParams("tab=help")).canonicalPath,
    "/settings?view=help"
  )
  assert.equal(
    resolveSettingsLocation(
      new URLSearchParams("view=publish-storage&section=publish")
    ).canonicalPath,
