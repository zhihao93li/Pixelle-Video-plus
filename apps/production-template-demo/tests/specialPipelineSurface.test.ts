...
test("special route resolves the exact recipe and canonicalizes legacy routes", () => {
  ...
  if (exact.kind === "ready") {
    ...
    assert.equal(exact.canonicalPath, "/create/special/image_to_video/my_i2v")
    assert.equal(exact.legacyRoute, false)
  }

  const legacy = resolveSpecialTemplate([custom, builtin], "image_to_video")
  assert.equal(legacy.kind, "ready")
  if (legacy.kind === "ready") {
    assert.equal(legacy.template.id, "pixelle_i2v_basic_v1")
    assert.equal(legacy.legacyRoute, true)
  }
})
