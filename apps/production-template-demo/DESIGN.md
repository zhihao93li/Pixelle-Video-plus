...
workflow / provider / runtime 级别的选项（画面 workflow、TTS workflow、生图提示词三项、模板默认生成配置编辑）只在专家模式（设置页开关，`lib/expertMode.ts`）下渲染。新增此类字段时必须用 `useExpertMode()` 包住，不允许默认暴露。**例外：配音引擎（tts_inference_mode）常驻显示**（2026-07-08 用户决定：引擎决定音色 ID 的填法，藏起来会让音色框变谜语）。
...
正式产品只保留一套 AppShell 和五项主导航：工作台、快速生产、任务、作品库、设置。独立 `/demo/studio` 与重复壳层已删除；选定的 split-canvas 参考图仅作为设计 QA 基准保留，不进入正式业务页面。
