...
export function SpecialPipelinesWorkspace({
  ...
  const canonicalPath =
    resolution?.kind === "ready" && resolution.legacyRoute
      ? resolution.canonicalPath
      : null

  useEffect(() => {
    if (canonicalPath) {
      window.history.replaceState(null, "", routeHref(canonicalPath))
      window.dispatchEvent(new HashChangeEvent("hashchange"))
    }
  }, [canonicalPath])

  if (loadState === "loading" || canonicalPath) {
    return (
      <SpecialStateShell mode={initialMode}>
        <AsyncState
          ...
          state="loading"
          title={canonicalPath ? "正在打开配方" : "正在读取配方"}
        />
      </SpecialStateShell>
    )
  }
