...
export function resolveSettingsLocation(
  ...
  const legacySection = query.get("section")
  const template = query.get("template")
  let view: SettingsView
  let focus = query.get("focus") || legacySection

  if (explicitView && VIEW_SET.has(explicitView)) {
    view = explicitView as SettingsView
  } else if (legacyTab === "templates" || template) {
    view = "recipes"
  } else if (legacyTab === "help") {
    view = "help"
  } else {
...
function normalizeFocus(view: SettingsView, focus: string | null) {
  ...
  if (view === "publish-storage") {
    if (focus === "publish") return "buffer"
