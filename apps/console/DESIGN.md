...
- 项目保存品牌信息、语言音色、发布默认和默认生产配方；不保存可执行的写稿 Prompt 或模型。
- 生产配方是从选题写稿、分镜到成片的唯一配置主体。审核流在起草前锁定配方，提交时不得静默换配方。
...
- 取消批次只终止尚未完成的子任务；已完成结果必须保留并继续可访问。
...
配方详情是设置页面，不是流程演示：按“文案生成、内容处理、画面生成、配音、版式与输出”直接展示可编辑字段，不使用逐项“调整”模式，也不重复展示相同生产步骤。Provider 是第一层选择；Workflow 只在对应 Provider/引擎需要时作为条件字段出现。原始路径与内部参数只进入专家设置。
...
项目只选择默认配方；可执行设置只有两层：配方生效默认 → 本次覆盖。写稿 Prompt、分镜 Prompt、写稿模型、分镜模型和每语言写稿模型全部属于配方。只有真正修改过的键进入 `overrides` 和 `dirtyKeys`。恢复默认是删除覆盖；显式清空继承值使用 `null`。

状态适配必须保留 `known | unknown` 区分。未知状态显示“状态待同步”，不能推断为运行中、终态或可取消。

生成任务由后端持久化。服务重启前未完成的任务进入 `interrupted`，不能丢失、恢复成排队中或误报为用户取消。内容生命周期只由后端根据这些任务推进，前端不得反向写状态进行 reconcile。
