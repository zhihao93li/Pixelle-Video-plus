...
- 多阶段设置使用阶段折叠面板：面板标题说明阶段职责和覆盖状态，一次聚焦一个阶段，避免把完整生产路线摊成长表。
- 阶段内使用两列字段网格；相关短字段并排，提示词、模型组合、模板选择和长文本等复杂字段独占整行。窄屏统一折叠为单列。
- 默认继承状态在阶段标题汇总，不在每个字段重复显示。字段旁只标记“未保存”“已自定义”等真实差异，并提供就近恢复入口。
- Provider 与模型、提示词与编辑动作、语言与语言模型等强关联控件可以在同一控件列内组合；组合内部使用稳定宽度，不能让一个控件缩成小块、另一个无限拉长。
- 普通设置控件使用一致的可读最大宽度；画面模板、长文本等确需更大空间的控件可以使用更宽展示，但不能改变其他设置行的对齐基准。
