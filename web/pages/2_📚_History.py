...
def render_publish_record(record: dict | None):
    ...
    for job in jobs:
        ...
        error = job.get("error")
        st.markdown(
            f"{icon} **{platform}**: `{status}` | Buffer post: `{post_id}` | due: `{due_at}`"
        )
        if error:
...
def render_publish_panel(task_id: str, pixelle_video, metadata: dict):
    ...
    with col_check:
        if st.button(
            ...
            checks = run_async(
                publish_manager.check_configuration(selected_platforms or platform_options)
            )
            for check in checks:
                if check.get("ok"):
...
def render_sidebar_controls(pixelle_video):
    ...
    with st.sidebar:
        ...
        page_size = st.selectbox(
            tr("history.page_size"), options=[15, 30, 60], index=0, key="page_size"
        )
...
def render_grid_task_card(task: dict, pixelle_video):
    ...
    with st.container():
        ...
        # Meta info (one line)
        st.caption(
            f"🕒 {format_datetime(created_at)} | ⏱️ {format_duration(duration)} | 🎬 {n_frames}"
        )
        ...
        with col2:
            if video_path and os.path.exists(video_path):
                ...
                st.button(
                    "⬇️", key=f"download_disabled_{task_id}", disabled=True, use_container_width=True
                )
...
def render_task_detail_modal(task_id: str, pixelle_video):
    ...
    with col_input:
        ...
        st.markdown(
            f"**{tr('history.detail.tts_mode')}:** {input_params.get('tts_inference_mode', 'N/A')}"
        )
    ...
    with col_video:
        ...
        if video_path and os.path.exists(video_path):
            ...
            st.markdown(
                f"**{tr('info.file_size')}:** {format_file_size(result.get('file_size', 0))}"
            )
...
def main():
    ...
    if total_pages > 1:
        ...
        with col1:
            if st.button(
                "⬅️ Previous", disabled=st.session_state.history_page == 1, use_container_width=True
            ):
                st.session_state.history_page -= 1
                st.rerun()
