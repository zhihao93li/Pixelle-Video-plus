...
def _completed_generation_task(video_path: str):
    ...
    return GenerationTask(
        ...
        entry="script",
        status="completed",
        progress=GenerationProgress(stage="completed", percentage=100.0),
        request=request,
        result=result,
    )
...
@pytest.mark.asyncio
async def test_plugin_request_generation_uses_unified_generation_service(tmp_path, monkeypatch):
    video_path = tmp_path / "plugin-unified.mp4"
    video_path.write_bytes(b"fake video bytes")
    generation_service = FakeGenerationService(_completed_generation_task(str(video_path)))
    store = OpsStore(tmp_path / "ops.db")
    store.init_db()
    ops_service = OpsService(store)

    async def fake_get_generation_service():
        return generation_service

    monkeypatch.setattr(server, "_build_service", lambda: ops_service)
    monkeypatch.setattr(server, "_get_plugin_generation_service", fake_get_generation_service)
    ...
    generated = await server.pixelle_request_generation(
        experiment_id=experiment["entity"]["id"],
    ...
    assert generated["content_item"]["asset_ref"]["generation_task_id"] == "gen-task-1"
    assert generated["event"]["payload"]["generation_task_id"] == "gen-task-1"
    assert generation_service.submitted_requests[0].input == {"script": "Generate a video"}
