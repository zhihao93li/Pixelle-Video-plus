...
class StandardPipeline(LinearVideoPipeline):
    ...
    async def produce_assets(self, ctx: PipelineContext):
        ...
        direct_image_limit = 1
        if config.image_provider in ("aliyun_bailian", "volcengine_ark"):
            provider_config = getattr(config_manager.config.image_generation, config.image_provider)
            direct_image_limit = provider_config.concurrency_limit
        concurrent_limit = max(
            runninghub_concurrent_limit if is_runninghub else 1,
            direct_image_limit,
        )

        if concurrent_limit > 1:
            logger.info(f"🚀 Using parallel frame processing (max {concurrent_limit} concurrent)")
            ...
            tasks = [
                process_frame_with_semaphore(i, frame) for i, frame in enumerate(storyboard.frames)
            ]
