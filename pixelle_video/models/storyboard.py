...
class StoryboardConfig:
    ...
    tts_workflow: Optional[str] = (
        None  # TTS workflow filename (for ComfyUI mode, None = use default)
    )
    ...
    media_workflow: Optional[str] = (
        None  # Media workflow filename (image or video, None = use default)
    )
    image_provider: Optional[str] = None  # Direct image provider; None keeps workflow behavior
    image_model: Optional[str] = None  # Provider model identifier
...
class StoryboardFrame:
    ...
    image_prompt: Optional[str]  # Image generation prompt (can be None for text-only or video)

    # Source storyboard metadata
    scene_id: Optional[str] = None  # Stable caller-owned scene identifier
    planned_duration: Optional[float] = None  # Optional planning estimate; TTS owns actual duration
    ...
    composed_image_path: Optional[str] = (
        None  # Composed image path (with subtitles, for image type)
    )
...
class Storyboard:
    ...
    def is_completed(self) -> bool:
        ...
        return all(frame.video_segment_path is not None for frame in self.frames)
    ...
    def progress(self) -> float:
        ...
        if not self.frames:
            return 0.0
        completed = sum(1 for frame in self.frames if frame.video_segment_path is not None)
