                ...
                for node_id, node_output in result.outputs.items():
                    if "text" in node_output:
                        ...
                        if text_list and len(text_list) > 0:
                            ...
                            logger.debug(
                                f"Found description in outputs.text: {description[:100]}..."
                            )
                            break
                            ...
                            async with aiohttp.ClientSession() as session:
                                async with session.get(item["fileUrl"]) as resp:
                                    if resp.status == 200:
                                        ...
                                        logger.debug(
                                            f"Downloaded description from URL: {description[:100]}..."
                                        )
                                        break

            if not description:
                logger.error(
                    f"No text found in result. Status: {result.status}, Outputs: {result.outputs}, Texts: {result.texts}"
                )
