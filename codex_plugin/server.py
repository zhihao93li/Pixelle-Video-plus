...
async def _build_generation_service_ops() -> OpsService:
    service = _build_service()
    if (
        service.generation_service is None
        and service.generation_runner is None
        and service.available_pipelines is None
    ):
        service.generation_service = await _get_plugin_generation_service()
    return service
...
async def pixelle_list_generation_pipelines() -> dict[str, Any]:
    ...
    service = await _build_generation_service_ops()
...
async def pixelle_get_generation_status(experiment_id: str) -> dict[str, Any]:
    service = await _build_generation_service_ops()
    return await _run_tool(lambda: service.get_generation_status(experiment_id))
