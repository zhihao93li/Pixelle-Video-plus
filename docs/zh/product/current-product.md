...
4. 设置：按“准备情况、工作空间、服务连接、生产规则、支持”组织配置；服务连接再按 AI 大模型、图片与视频生成、语音生成、发布与存储划分。
...
设置概览只突出阻塞生产的必要项并提供直接处理入口，可选项和底层诊断收纳在高级区域。“已配置”只表示配置已保存，不代表外部服务实时连通；真实可用性以连接测试或任务执行结果为准。
