...
内容空间只用于归集和筛选一组内容、任务与作品，可以代表账号、品牌或工作方向。它不保存生产模板、语言、音色或发布平台默认值；人和 Agent 通过同一组用例接口建内容账和生产任务。
...
3. 作品库：用封面平铺浏览生成结果，支持多选下载；进入作品详情后预览、发布或跳转快速生产再次制作。作品详情复用同一份历史产物数据，但不复用生产任务详情页，因为前者管理已经产出的作品，后者处理生产状态、失败与人工确认。
4. 设置：管理内容空间、AI、语音、生成引擎、存储和模板。

React 只有「快速生产」可以创建新生产；工作台是监控和人工处理中心。任务详情只从工作台卡片进入，顶部展示唯一当前操作，下方按时间倒序展示真实输出与操作记录，不再跳转第二个「内容详情」。

工作台各栏任务卡统一按最近更新时间倒序。归档只是隐藏稳定生产任务卡，不改变生产状态，不删除内容、作品、文件或执行记录；已归档卡片可显示并恢复。正在运行或等待人工处理的任务必须先取消或放弃，不得用归档掩盖仍在进行的工作。

快速生产页的唯一提交按钮固定在视口底部，用户调整到页面末尾后无需返回顶部。首次使用提示只读取当前机器真实诊断结果，列出仍缺少的必要连接或本地工具；完成后自动消失，不用静态新手清单冒充能力检测。
...
批量生产只是同一条 Pipeline 的多任务提交方式，不是新的 Pipeline。文案批量用单独一行的 `---` 分隔任务，每个区块第一行是标题、其余内容是正文；图片批量则以每个上传文件作为一个任务。所有批量入口必须在输入区直接说明区分规则，并在提交前展示实际解析出的任务数量。
...
系统设置管理 Provider、凭据和系统级服务默认；模板管理一条路线的长期生产效果；前端只提交本次真正修改的覆盖项。后端负责校验、合并和生成最终运行参数。
...
同一生产的原样重试追加 execution attempt，不覆盖旧错误。人工确认站支持直接编辑或整套重新生成，并继续使用原生产任务、使旧确认失效；不提供由 LLM 局部重做若干分镜。删除非首个分镜时，其文案、画面提示词和时长合并进上一镜，再由用户保存或确认。已经产出后再改文案、素材、模板或有效参数，则从快速生产或 Agent 发起新任务，旧任务与旧产物保留。
