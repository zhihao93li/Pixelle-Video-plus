...
本轮适合做轻清理，不适合做大规模删代码或目录重构。根因是 Streamlit 迁移 gate 还没有关闭：Action Transfer 的有效用户素材真实 E2E 仍未通过，Buffer 真实发布仍按用户边界由人工确认。因此 `web/`、旧 Streamlit pipeline、legacy API 和对照脚本仍有保留价值。
...
| 优先级 | 项 | 建议 |
| --- | --- | --- |
| P1 | Action Transfer 有效素材真实 E2E 未通过 | 使用真实单人动作视频和目标人像复跑；不要再用 verifier 自动生成的黑色视频作为动作迁移参考输入 |
...
1. Action Transfer 有效素材真实 E2E 通过。
