...
后续 P3 发布准备：`docs/zh/product/pixelle-p3-publish-ui-design.md`
...
P3-A 发布准备不改变顶层导航，不新增 `Publish`。发布模块底层保持独立，但前端先扩展 Ops 第 5 步，把“发布并记录证据”升级为“发布准备与证据”。独立 Publish 页面只在发布包批量管理、跨项目检索、assisted/API 发布 attempt 成为常用能力后再评估。
...
| 5. 发布准备与证据 | `PublishPackage` 工作台：平台账号、内容类型、资产交付、标题/正文/标签/完整发布包复制块、发布 checklist、手动证据登记入口；如果是 mock，显示 mock 证据。 | 是否可以人工发布、package 是否锁定、copy blocks 是否完整、复制是否被误认为发布、真实证据是否缺失、是否阻塞指标观测和复盘。 | package_created/package_locked、PublishEvidence、PublishRecord、mock publish 事件、平台响应、发布说明或失败记录。 |
...
| 5. 发布准备与证据 | `PublishPackage`、资产交付、标题/正文/标签/完整发布包复制块、checklist、URL/post id/发布时间/mock 标记 | package 是否可用、是否缺必填 copy block、复制是否不等于发布、缺哪些真实证据、是否阻塞观测和复盘 | 视频时长、pipeline、完整草稿、预测假设、通用闭环进度、平台登录态、cookie、自动化脚本 |
