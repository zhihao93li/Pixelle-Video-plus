...
内容产生不能独立成为主产品。没有运营项目、运营周期和内容实验上下文的生成，只能是 sandbox，不得进入默认运营主线。
...
它表示一个持续运营的上下文，例如一个产品、账号、品牌或创作者主题。它持有长期目标、默认渠道、默认账号和项目记忆。
...
它可以有脚本、标题、caption、发布账号、状态和媒体资产，但它的运营意义来自所属的 ContentExperiment。
...
用户通过 Codex 手动描述候选内容，或让 Codex 基于上下文提议候选。
...
用户创建 PetWoods 小红书增长项目，开启“清洁内容验证”周期，选择一条内容实验，锁定预测，生成视频，人工发布后录入小红书链接，三天后录入数据并复盘。
