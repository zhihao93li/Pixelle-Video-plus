...
- **2026-07-08｜配方页兜底参数归位（批次三后续，用户点名直接实施）**：`PIPELINE_PARTS` 步骤新增 `subKeys` 附属参数（标准线：配音引擎挂 tts_workflow/语速、每镜画面挂画面模板/宽高/生图提示词三项、合成挂 BGM 三件套；素材线/数字人/图文线同理归位），配方详情页步骤卡下折叠渲染附属参数行（按模板白名单过滤，编辑器复用 renderEditor——画面模板自动是图选网格）；「更多可调参数」兜底区只留未归位参数（标准线归位后为空则整区隐藏），兜底机制保留防漏登记。coveredKeys 计算含 subKeys。与生成页产线分组同构闭环：两页同一张地图、同一批参数、同一个归属。`tsc -b`/lint/21 测试全绿。

- **2026-07-08｜批次三白屏修复 + typecheck 脚本纠正**：① 生成页首帧 `template` 为 null 时解引用 `pipeline_id` 致整树崩溃白屏——改 `template?.` 兜底。② 根因帮凶：`npm run typecheck` 原为 `tsc --noEmit`，对 `"files": []` 的引用壳 tsconfig **不检查任何文件**（历来的"typecheck 全绿"在该命令下为空转）；脚本已改 `tsc -b` 并 `--force` 全量复查通过。**守则更新：前端类型检查唯一可信命令 = `tsc -b`。**

- **2026-07-08｜视觉批次三：生成页产线同构 + 画面模板图选（prd-visual-batch3-generate-pipeline-alignment.md，本批由审核方直接实施）**：① 共享 `FrameTemplatePicker`（inline 图选网格：方向过滤、名字卡兜底、选中环、懒加载、前 12 格+展开、可选「使用模板默认」卡），三处接入——生成页「每镜画面」、配方详情页 frame_template 零件编辑（资源拉不到静默回落文本框）、出片面板专家覆盖（空串=撤销覆盖）。② 生成页高级设置产线同构：删「内容结构/画面风格/声音与音乐」三组，改为产线步骤分组（1 写稿惰性行「本次跳过」、2 分镜、3·4 配音、5 每镜画面、6 合成），组编号/标题/显隐全部经 `pipelineStepBadge`/`pipelinePartLabel` 从 PIPELINE_PARTS 查出（附录 B：零件表为唯一事实源，长文自动只剩写稿行、图文自动 2 分页+3·4 每页配图）；标题字段移出分组为主料；生图提示词三项收进专家行并统一零件表命名；BGM 移入合成组，组尾指路「合成引擎在配方里换」；AdvancedGroup 加 step 编号胶囊。③ 来源标注 v1（脏值比较 vs 静态默认）：核心字段标签旁「本次」Badge / 灰字「配方默认」，提交按钮上方总结「本次覆盖 n 项（…）/全部沿用配方默认」。④ **payload 零变化**：`buildStandardTemplateInput` 与 StandardAdvancedSettings 形状未动，专家隐藏字段值保留照常提交。DESIGN.md §2.7 补排版参照条款与进度。三件套全绿；本地待验：build + 视频/图文/长文各出一条 + 批量与出片面板回归。
