...
5. Confirm the required production templates are enabled.
...
1. Open Quick Production.
2. Choose video, image set, long-form text, or a specialized video flow.
3. Select a template and open its generation workspace.
4. Enter a script, topic, or source assets.
5. Review the estimate and current overrides, then start generation.
6. Follow progress in the workbench task detail.
7. Preview the result in Library and continue to publishing when eligible.

Batch is a submission mode, not a separate page. Switch to Batch inside a template that supports it; each item is tracked independently.
