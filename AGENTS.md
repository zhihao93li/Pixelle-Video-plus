...
4. `confirm` 是人工闸门，可以在 React 或受支持的 Agent 对话中完成。Agent 只有在展示完整待确认版本、获得用户明确确认后，才能传入精确的 `content_version` 和 `explicit_user_confirmation=true`；模糊态度不能视为确认。
...
4. 需要确认时，展示完整内容和 `updated_at` 版本；用户明确确认后调用 `confirm_pending_item`
5. 用户要修改时，用 `edit_pending_review` 直接保存，或用 `regenerate_pending_review` 重写/局部重做/整套重做；始终重新读取最新版本再确认
...
1. 用 `start_production` 提交 1–20 个纯文案分镜和图片提示词，建立稳定生产任务
